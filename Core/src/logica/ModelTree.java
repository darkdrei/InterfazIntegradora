...
public class ModelTree implements TreeModel{
	...
	public ModelTree( Class<?> root, Set<Extension> extensions ) throws InstantiationException, IllegalAccessException{
	...
	public int getChildCount( Object parent ){
		ModelTree.Node[] children = ((Node)parent).children;
		if( children == null ){
			return 0;
		}
	...
	public class Node{
		...
		public Node( Class<?> clazz ) throws InstantiationException, IllegalAccessException{
			...
			if( BaseSet.class.isAssignableFrom( clazz )){
				BaseSet set = (BaseSet) clazz.newInstance();
		...
		public boolean isLeaf(){
			return !BaseSet.class.isAssignableFrom( clazz );
		}
		
		public String getTitle(){
			if( !titleSet ){
				titleSet = true;
				Anotacion tutorial = (Anotacion)clazz.getAnnotation(Anotacion.class );
				if( tutorial == null ){
